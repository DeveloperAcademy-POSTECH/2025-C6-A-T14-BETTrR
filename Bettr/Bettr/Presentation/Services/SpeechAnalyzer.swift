...
class SpeechAnalyzer {
    ...
    // MARK: - Smith–Waterman으로 가장 유사한 구간 찾기
    private func smithWatermanRegion(reference: [String], recognized: [String]) -> (Int, Int) {
        ...
        for i in 1...n {
            for j in 1...m {
                let s = (reference[i-1] == recognized[j-1]) ? match : mismatch
                dp[i][j] = max(0,
                               dp[i-1][j-1] + s,
                               dp[i-1][j] + gap,
                               dp[i][j-1] + gap)
        ...
        while i > 0 && j > 0 && dp[i][j] > 0 {
            let s = (reference[i-1] == recognized[j-1]) ? match : mismatch
            if dp[i][j] == dp[i-1][j-1] + s { i -= 1; j -= 1 }
            else if dp[i][j] == dp[i-1][j] + gap { i -= 1 }
            else if dp[i][j] == dp[i][j-1] + gap { j -= 1 }
            else { break }
        }
        return (i, endPos.i)
    }
    
    // MARK: - Levenshtein 거리 기반 단어 비교
    private func computeLevenshtein(reference: [String], recognized: [String]) -> [WordDiff] {
        ...
        while i > 0 || j > 0 {
            if i > 0 && j > 0 && reference[i - 1] == recognized[j - 1] {
                diffs.insert(.matched(word: reference[i - 1]), at: 0)
                i -= 1
                j -= 1
            } else if i > 0 && j > 0 && dp[i][j] == dp[i - 1][j - 1] + 1 {
                diffs.insert(.replaced(expected: reference[i - 1], actual: recognized[j - 1]), at: 0)
                i -= 1
                j -= 1
            } else if i > 0 && dp[i][j] == dp[i - 1][j] + 1 {
                diffs.insert(.missing(expected: reference[i - 1]), at: 0)
                i -= 1
            } else if j > 0 && dp[i][j] == dp[i][j - 1] + 1 {
                diffs.insert(.extra(actual: recognized[j - 1]), at: 0)
                j -= 1
            } else {
                break
            }
    ...
    // MARK: - 최종 분석 (Local-to-Global 하이브리드)
